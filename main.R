...
summarize_matrix <- function(x, na.rm=FALSE) {
  row_mean <- apply(x,1,mean,na.rm=na.rm)
  row_stdev <- apply(x,1,sd,na.rm=na.rm)
  row_median <- apply(x,1,median,na.rm=na.rm)
  row_min <- apply(x,1,min,na.rm=na.rm)
  row_max <- apply(x,1,max,na.rm=na.rm)
